...
#else
#define CODE_GEN_ALIGN           4 /* under CE don't waste memory */
#endif
#ifdef UNDER_CE
#define CODE_GEN_PHYS_HASH_BITS     16
#else
...
#if defined(DYNAMIC_TB)
extern int CODE_GEN_BUFFER_SIZE;
#else
...
#define CF_SINGLE_INSN 0x0008 /* compile only a single instruction */
    ...
    struct TranslationBlock *next_tb;
    struct TranslationBlock *prev_tb;
} TranslationBlock;
...
void CacheRangeFlush (void* pAddr, unsigned int dwLength, unsigned int dwFlags);

static inline void tb_set_jmp_target1(unsigned long jmp_addr, unsigned long addr)
{
    /* patch the branch destination */
    *(uint32_t *)jmp_addr = 0xEA000000+(((addr-jmp_addr)/4-2)&0xffffff);
//    CacheRangeFlush((void*)jmp_addr,4,CACHE_SYNC_L2_WRITEBACK|CACHE_SYNC_WRITEBACK|CACHE_SYNC_INSTRUCTIONS|CACHE_SYNC_FLUSH_I_TLB);
    CacheRangeFlush((void*)jmp_addr,4,CACHE_SYNC_INSTRUCTIONS|CACHE_SYNC_WRITEBACK|CACHE_SYNC_FLUSH_TLB);
}
#endif
...
#elif defined(UNDER_CE) && defined(USE_DIRECT_JUMP)
