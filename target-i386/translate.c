...
static void gen_op_set_cc_op(int op)
{
#define MAKE_CC_OP(reg_op) 	\
    if(op==reg_op)	\
         glue(gen_op_set_cc_op_,reg_op)();	\
    else
MAKE_CC_OP(CC_OP_DYNAMIC)
MAKE_CC_OP(CC_OP_EFLAGS)
MAKE_CC_OP(CC_OP_MULB)
MAKE_CC_OP(CC_OP_MULW)
MAKE_CC_OP(CC_OP_MULL)
MAKE_CC_OP(CC_OP_MULQ)
MAKE_CC_OP(CC_OP_ADDB)
MAKE_CC_OP(CC_OP_ADDW)
MAKE_CC_OP(CC_OP_ADDL)
MAKE_CC_OP(CC_OP_ADDQ)
MAKE_CC_OP(CC_OP_ADCB)
MAKE_CC_OP(CC_OP_ADCW)
MAKE_CC_OP(CC_OP_ADCL)
MAKE_CC_OP(CC_OP_ADCQ)
MAKE_CC_OP(CC_OP_SUBB)
MAKE_CC_OP(CC_OP_SUBW)
MAKE_CC_OP(CC_OP_SUBL)
MAKE_CC_OP(CC_OP_SUBQ)
MAKE_CC_OP(CC_OP_SBBB)
MAKE_CC_OP(CC_OP_SBBW)
MAKE_CC_OP(CC_OP_SBBL)
MAKE_CC_OP(CC_OP_SBBQ)
MAKE_CC_OP(CC_OP_LOGICB)
MAKE_CC_OP(CC_OP_LOGICW)
MAKE_CC_OP(CC_OP_LOGICL)
MAKE_CC_OP(CC_OP_LOGICQ)
MAKE_CC_OP(CC_OP_INCB)
MAKE_CC_OP(CC_OP_INCW)
MAKE_CC_OP(CC_OP_INCL)
MAKE_CC_OP(CC_OP_INCQ)
MAKE_CC_OP(CC_OP_DECB)
MAKE_CC_OP(CC_OP_DECW)
MAKE_CC_OP(CC_OP_DECL)
MAKE_CC_OP(CC_OP_DECQ)
MAKE_CC_OP(CC_OP_SHLB)
MAKE_CC_OP(CC_OP_SHLW)
MAKE_CC_OP(CC_OP_SHLL)
MAKE_CC_OP(CC_OP_SHLQ)
MAKE_CC_OP(CC_OP_SARB)
MAKE_CC_OP(CC_OP_SARW)
MAKE_CC_OP(CC_OP_SARL)
MAKE_CC_OP(CC_OP_SARQ)
MAKE_CC_OP(CC_OP_NB)
	{
		puts("Unknown CC_OP");
		exit(-1);
...
#ifdef UNDER_CE
static void gen_hook(DisasContext *s, target_ulong cur_eip, void* addr)
{
    if (s->cc_op != CC_OP_DYNAMIC)
        gen_op_set_cc_op(s->cc_op);
    gen_jmp_im(cur_eip);
    gen_op_hook((int)addr);
    s->is_jmp = 3;
}
#endif
...
#ifdef UNDER_CE
	if((Hook=DoPatch((get_phys_addr_code(env, pc_ptr) & TARGET_PAGE_MASK), pc_ptr, dc->cs_base)) != 0)
	{
    	gen_hook(dc, pc_ptr - dc->cs_base,Hook);
	} 
#endif       
        if (search_pc) {
        ...
        if (gen_opc_ptr >= gen_opc_end ||
            (pc_ptr - pc_start) >= (TARGET_PAGE_SIZE*16)) {
            gen_jmp_im(pc_ptr - dc->cs_base);
            gen_eob(dc);
            break;
        }
...
#ifdef UNDER_CE
    if((cflags & CF_SINGLE_INSN)==0 || gen_opc_ptr - gen_opc_buf<4)
#endif
