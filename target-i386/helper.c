 ...
 * is_int is TRUE.
 */
static void __chk1(int intno, int error_code)
{
...
void helper_fxsave(target_ulong ptr, int data64)
{
    ...
    for(i = 0;i < 8; i++) {
	    ...
	    temp.d = ST(i);
	    stq(addr, temp.l.lower);
    	stw(addr + 8, temp.l.upper);
#else
        tmp = ST(i);
...
void my_stl_kernel(unsigned int addr, unsigned int data)
{
	void* prev=(void*)env;
	env = cpu_single_env;
	stl_kernel(addr, data);
	env=(struct CPUX86State*)prev;
}
void my_stw_kernel(unsigned int addr, unsigned short data)
{
	void* prev=(void*)env;
	env = cpu_single_env;
	stw_kernel(addr, data);
	env=(struct CPUX86State*)prev;
}
