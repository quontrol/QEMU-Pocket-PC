...
void OPPROTO op_hook(void)
{
    env->exception_index = EXCP_HOOK;
    env->error_code = PARAM1;
    cpu_loop_exit();
}
...
void OPPROTO glue(op_set_cc_op_,reg_op)(void)\
{	\
    CC_OP = reg_op;	\
}
...
#ifdef UNDER_CE
void OPPROTO op_the_last_function_in_file(void)
{
    helper_invlpga();
}
#endif
