...
void sym_ent_name(struct external_syment *ext_sym, EXE_SYM *sym)
{
    ...
    /* WinCE GCC does not prepend underscore */
...
int load_object(const char *filename)
{
    ...
    struct external_syment *sym_follow, *sym_next = 0;
    ...
    if (fhdr.f_magic != I386MAGIC && fhdr.f_magic != ARMCEMAGIC) {
    	printf("%d ",fhdr.f_magic);
	...
	for (i = 0, ext_sym = coff_symtab, sym = symtab; i < nb_syms; i++, ext_sym++, sym++) {
...
#ifdef HOST_ARM_CROSS
		if(1)	// we still incorrectly determine the end of some functions. Fortunately they become larger.
#else
		if(sym->st_size==0)
#endif
		{
		...
		/* Find the following symbol in order to get the current symbol size */
	        for(j = 0, sym_follow = coff_symtab; j < nb_syms; j++, sym_follow++) {
    	        if ( (sym_follow->e_scnum == text_shndx && (*(uint8_t *)sym_follow->e_sclass == C_EXTERNAL
#ifdef HOST_ARM_CROSS
				|| 1 
#endif
    	        )) && 
    	        sym_follow->e_value > sym->st_value && sym_follow->e.e_name[0]!='L'
#ifdef HOST_ARM_CROSS
				&& ((sym_follow->e.e.e_zeroes==0 && (strtab + sym_follow->e.e.e_offset)[0]!='L') || (sym_follow->e.e.e_zeroes && sym_follow->e.e_name[0]>32))
#endif
    	        )
        	    {
		            if(!sym_next) {
	            	...
	            	if(sym_follow->e_value < sym_next->e_value)
		            	sym_next = sym_follow;
		        }
	        }
			if(sym_next)
    	        sym->st_size = sym_next->e_value - sym->st_value;
//			else
//            	sym->st_size = text_sec_hdr->size - sym->st_value;
//			if(sym->st_name[0]!='L')
//			if(sym_next)
//				printf("'%s' - '%s' (%d) %x %x\n",sym->st_name,sym_next->e.e_name,sym->st_size,sym->st_value,sym_next->e_value);
		}
	...
	*sslide = 0;
	...
	switch(rel->r_type)
	{
		...
		case PPC_RELOC_BR24:
			...
			if (sectoffset & 0x0200    ) sectoffset |= 0xfc00    ;
			break;
		default:
              ...
              FILE *outfile, int gen_switch)
{
    ...
    p_end = p_start + size;
    start_offset = offset;
#if defined(HOST_I386) || defined(HOST_X86_64) || defined(HOST_ARM_CROSS)
#ifdef CONFIG_FORMAT_COFF
#if defined(HOST_ARM_CROSS)
    {
        uint32_t *p;
        p = (uint32_t*)(p_end - 4);
        if(*p==0xe12fff1e)
	        copy_size = p_end - p_start -4;
	    else
...
#define INSN_SUB_SP     0x9c23a000 // sub %sp, nn, %sp
...
#elif defined(HOST_ARM)
    {
        ...
        if (get32((uint32_t *)p_start) != 0xe1a0c00d ||
            (get32((uint32_t *)(p_start + 4)) & 0xffff0   ) != 0xe92d0    ||
...
#error unsupported CPU
    ...
    if (gen_switch == 2) {
#ifdef HOST_ARM_CROSS
/* For WinCE I prepend two commands for each generated function. So it grows on 8 bytes */
        uint32_t *p;
        uint32_t *s;
        p = (uint32_t*)(p_end - 4);
        s = (uint32_t*)(p_start);
	    ...
	    else
	    {
	    	if(p[-1]==0xe12fff1e)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-2]==0xe12fff1e && copy_size>12)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-3]==0xe12fff1e && copy_size>16)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-4]==0xe12fff1e && copy_size>20)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else
	    	if(p[-1]==0xe49DF004 && s[0]==0xe52de004)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-2]==0xe49DF004 && copy_size>12 && s[0]==0xe52de004)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-3]==0xe49DF004 && copy_size>16 && s[0]==0xe52de004)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else if(p[-4]==0xe49DF004 && copy_size>20 && s[0]==0xe52de004)
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
	    	else
		        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size+((copy_size/4-1)>255?8:4));
		}
#else
        fprintf(outfile, "DEF(%s, %d, %d)\n", name + 3, nb_args, copy_size);
#endif
...
#else
#ifndef HOST_ARM_CROSS
/* under WinCE all externs are already resolved by linker */
                    fprintf(outfile, "extern char %s;\n", sym_name);
#endif
#endif
                }
...
#ifdef HOST_ARM_CROSS
/* for WinCE I prepend code:
   BL function
   B function+function_size 
function: ..the code of op_ function..   */
   {
        uint32_t *p;
        uint32_t *s;
        p = (uint32_t*)(p_end - 4);
        ...
        if(*p!=0xe12fff1e)
        {
        	if(p[-1]==0xe12fff1e)   // BX LR; DD constant -> B PC (after memcpy)
        	{
        		;
        	} else if(p[-2]==0xe12fff1e && copy_size>12)
			{
        	...
        	if(p[-1]==0xe49df004 && s[0]==0xe52de004)   //LDR PC,[sp],4
        	{
        		;
        	} else if(p[-2]==0xe49df004 && copy_size>12 && s[0]==0xe52de004)
			{
        	...
        	{
	        	if((copy_size/4-1)>255)
    	    	{
			   		fprintf(outfile, "    ((uint32_t*)gen_code_ptr)[0]=0xEB000000;\n");  // BL
			   		fprintf(outfile, "    ((uint32_t*)gen_code_ptr)[1]=0x%08X;\n",0xEA000000+copy_size/4-1); // B
				    fprintf(outfile, "    gen_code_ptr += 8;\n");
	        	} else
    	    	{
				    fprintf(outfile, "    ((uint32_t*)gen_code_ptr)[0]=0x%08X;\n",0xE28FEF00+(copy_size/4-1)); // ADD LR,PC,constant*4
...
#ifdef HOST_ARM_CROSS
{
        ...
        s = (uint32_t*)(p_start);
      	if(p[-1]==0xe49df004 && s[0]==0xe52de004)   //LDR PC,[sp],4
       	{
	        fprintf(outfile, "    memcpy(gen_code_ptr, (void *)(((char *)&%s)+%d), %d); *((uint32_t*)gen_code_ptr)=0xe1a00000;\n",
        ...
        fprintf(outfile, "    memcpy(gen_code_ptr, (void *)((char *)&%s+%d), %d);\n",
...
#ifdef HOST_ARM_CROSS
 		{
	        ...
	        s = (uint32_t*)(p_start);
	       	...
	       	if(p[-1]==0xe12fff1e)   // BX LR; DD constant -> B PC
	    	    fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d - 8) = 0xEA000000;\n",copy_size);
	    	else if(p[-2]==0xe12fff1e && copy_size>12)
	    	    fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d - 12) = 0xEA000001;\n",copy_size);
	    	else if(p[-3]==0xe12fff1e && copy_size>16)
	    	    fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d - 16) = 0xEA000002;\n",copy_size);
	    	else if(p[-4]==0xe12fff1e && copy_size>20)
	    	    fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d - 20) = 0xEA000003;\n",copy_size);
        ...
        /* patch relocations */
#if defined(HOST_I386) || defined(HOST_ARM) || defined(HOST_ARM_CROSS)
            {
                char relname[256];
                int type;
                int addend;
		    ...
		    rel->r_offset < start_offset + copy_size) {
                    ...
                    if (strstart(sym_name, "__op_jmp", &p) || strstart(sym_name, "_op_jmp", &p) || strstart(sym_name, "___op_jmp", &p)) {
                        int n;
                        n = strtol(p, NULL, 10);
                    ...
                    switch(type) {
                    case 1:	// arm-wince-pe
						/* under WinCE all fixups except __op_param are already resolved by linker */
						if(strstart(sym_name, "__op_param", NULL))
    	                    fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d) = %s + %d;\n", 
	                            rel->r_offset - start_offset, relname, addend);
                    ...
                    case DISP32:
                        fprintf(outfile, "    *(uint32_t *)(gen_code_ptr + %d) = %s - (long)(gen_code_ptr + %d) + %d -4;\n",
                                reloc_offset, relname, reloc_offset, addend);
                        break;
                    case 3: // arm-wince-pe, pc-relative relocs
                        fprintf(outfile, "    arm_reloc_pc24((uint32_t *)(gen_code_ptr + %d), 0x%x, %s);\n",
                                reloc_offset, addend, name);
                        break;
                    default:
                        error("unsupported i386 relocation (%d),'%s'", type,name);
                    }
#else
