...
#if (defined(_WIN32) || defined(__APPLE__)) && !defined(UNDER_CE)
...
#elif defined(__arm__)
#ifdef UNDER_CE
/* On WinCE avoid using 26-bit relocations, until they would be implemented in dyngen */
#define EXIT_TB() asm volatile (  \
"ldr pc,[pc,#-4]\n" \
".word exec_loop\n" : : : "memory")
#define GOTO_LABEL_PARAM(n) asm volatile (  \
"ldr pc,[pc,#-4]\n" \
 ".word " ASM_NAME(__op_gen_label) #n "\n" : : : "memory")
#else
